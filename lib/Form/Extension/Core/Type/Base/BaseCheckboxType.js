...
class BaseCheckboxType extends BaseType {
    constructor(props) {
        ...
        this.addDataValidationRule('Choice', {
            'choices': [null, this.getOption('value')],
            'multiple': false,
            'strict': false
        });
    ...
    onChange = () => {
        let value = (this.state.value == this.getOption('value')) ? null : this.getOption('value');
    ...
    // Rendering -----------------------------------------------------------------------------------
    createField() {
        let _checkType = (this.state.value == this.getOption('value')) ? 'check-box' : 'check-box-outline-blank';
